...
export default function EditProject(props: { params: Promise<{ id: string }> }) {
    ...
    const fetchProject = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                showToast('error', 'Gagal memuat', err.error || 'Project tidak ditemukan')
                return
            }
            const { project } = await res.json()
            const stack: string[] = Array.isArray(project.techStack) ? project.techStack : []
            const imgs: string[] = Array.isArray(project.imageUrls) ? project.imageUrls : []
            setFormData({
                title: project.title || '',
                slug: project.slug || '',
                ...
                status: (project.status as ProjectForm['status']) || 'Live',
            })
            setImageUrls(imgs)
        } catch (err: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetchCsrf(`/api/admin/projects/${params.id}`, {
                method: 'PUT',
            ...
            if (!res.ok) {
                const err = await res.json().catch(() => ({}))
                showToast('error', 'Update Failed', err.error || 'Gagal memperbarui project')
                return
            }
